...
// Utiliza la URL de mongoDB según se quiera instalar la base de datos normal o la base de datos de test
const dbURL =
  process.argv[2] === process.env.DB_INSTALL_CLASSIC_MODE
...
db.connect(dbURL).then(async conn => {
  try {
    await installDB();
    closeDBConnection();
  } catch (error) {
